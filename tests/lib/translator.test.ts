...
import { initTranslator } from '../../src';
...
describe('translator', () => {
  ...
  describe('initTranslator', () => {
    it('Should configure i18n and return instance after first time', () => {
      const result = initTranslator('tests/assets');
      initTranslator('tests/assets'); // translator is a singleton, so initTranslator should still be haveBeenCalledTimes = 1
  ...
  describe('translate', () => {
    const translator = initTranslator('tests/assets');
